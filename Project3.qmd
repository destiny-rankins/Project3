...
-   **Results:** The Brier Score obtained after assessing the performance of the logistic regression model on the source population was 0.147 The estimated Brier risk from the underlying NHANES target population was 0.103. The estimated Brier risk from the simulated target population was 0.026.
...
model_fram <- glm(CVD~log(HDLC)+log(TOTCHOL)+log(AGE)+log(SYSBP_UT+1)+
                 log(SYSBP_T+1)+CURSMOKE+DIABETES,
      data= train_framingham_df, family= "binomial")

# Calculate predicted probabilities for the source test set
fram_predict_probs <- predict.glm(model_fram, newdata = test_framingham_df, type = "response")
...
framnhanes_train_df  <- combined_framnhanes[sample_framnhanes, ]
...
model_framnhanes <- glm(S~log(HDLC)+log(TOTCHOL)+log(AGE)+log(BMI)+
      ...
      data= framnhanes_train_df, family= "binomial")
...
brierscores <- data.frame(Population = c("Framingham", "NHANES", "Sim Target"), "Brier Score" = c(round(test_brier,3), paste(round(framnhanes_brier_estimate,3), "*"), paste(round(sim_brier_estimate,3), intToUtf8(8224))))
